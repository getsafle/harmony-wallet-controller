...
const { KeyringController: HarmonyKeyring, getBalance } = require('../src/index')
...
describe('Initialize wallet ', () => {
    ...
    it("Get fees with manual gasLimit", async () => {
        const accounts = await harmonyKeyring.getAccounts()
        ...
        const privateKey = await harmonyKeyring.exportAccount(accounts[0])
        const tx3 = await harmonyKeyring.sign(TESTING_MESSAGE_1, privateKey, web3)
        console.log("tx3 ", tx3)
    })

    it("Should import correct account ", async () => {
        const address = await harmonyKeyring.importWallet(EXTERNAL_ACCOUNT_PRIVATE_KEY)
        assert(address.toLowerCase() === EXTERNAL_ACCOUNT_ADDRESS.toLowerCase(), "Wrong address")
        assert(harmonyKeyring.importedWallets.length === 1, "Should have 1 imported wallet")
    })
